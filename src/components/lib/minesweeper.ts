...
function isVictorious(game: Game) {
  const unknownCells = Object.keys(game.cellsByXy).filter(
    (xy) => !isCellKnown(game.cellsByXy[xy])
  );
...
function flag(game: Game, xy: string): Game {
  ...
  if (cell.status === CellStatus.Unknown) {
    ...
    const unknownNeighbors = game.neighborsByXy[xy].filter(
      (nxy) => game.cellsByXy[nxy].status === CellStatus.Unknown
    );
